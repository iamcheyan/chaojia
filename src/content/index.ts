...
if (typeof (window as any).__CHAOJIA_LOADED__ === 'undefined') {
  ...
  if (window.parent !== window) {
    ...
    // Get the adapter for this site - exit gracefully if not supported
    let siteAdapter: ChatSiteAdapter | undefined
    try {
      siteAdapter = getActiveChatSiteAdapter()
    } catch (err) {
      console.log('[MultiChat] This site is not supported, content script inactive:', (err as Error).message)
    }
