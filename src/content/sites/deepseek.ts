...
const EDITOR_SELECTORS =
  'textarea[placeholder], div[contenteditable="true"][role="textbox"], textarea[contenteditable="true"]'

const SEND_BUTTON_SELECTORS =
  'button[aria-label*="发送"], button[aria-label*="Send"], button[type="submit"], button[aria-label*="送信"], button[aria-label*="送る"]'

const NEW_CHAT_SELECTORS =
  'a[href="/new"], button[aria-label*="新对话"], button[aria-label*="New Chat"], button[aria-label*="新しいチャット"], a[aria-label*="新对话"]'

const RESPONSE_SELECTORS = '.markdown, .prose, [data-testid="message"], [class*="message"], [class*="response"]'

const STOP_SELECTORS =
  'button[aria-label*="停止"], button[aria-label*="Stop"], button[aria-label*="中止"], button[aria-label*="ストップ"]'

const ACTIVITY_INDICATORS =
  '.result-streaming, [data-is-streaming="true"], .animate-pulse, [class*="loading"], [class*="typing"]'
...
export function createDeepSeekAdapter(): ChatSiteAdapter {
  return {
    id: 'deepseek',
    ...
    async startNewChat(): Promise<boolean> {
      ...
      location.href = 'https://chat.deepseek.com/new'
      return true
    },
