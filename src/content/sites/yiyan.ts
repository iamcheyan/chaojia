...
const EDITOR_SELECTORS =
  '#search-input-box textarea, textarea[placeholder], div[contenteditable="true"][role="textbox"]'

const SEND_BUTTON_SELECTORS =
  'button[aria-label*="发送"], button[aria-label*="发送"], button[aria-label*="Send"], #search-input-box button[type="submit"], div[class*="search-btn"]'

const NEW_CHAT_SELECTORS =
  'a[href="/"], button[aria-label*="新建对话"], button[aria-label*="新对话"], a[href*="yiyan.baidu.com"]'

const RESPONSE_SELECTORS = '.chat-content, .assistant-content, .markdown-body, .response-content, div[class*="message-content"]'

const STOP_SELECTORS =
  'button[aria-label*="停止"], button[aria-label*="Stop"], button[class*="stop"]'

const ACTIVITY_INDICATORS =
  '.loading, .generating, [data-loading="true"], .animate-pulse, div[class*="loading"]'
...
export function createYiyanAdapter(): ChatSiteAdapter {
  return {
    id: 'yiyan',
    ...
    async startNewChat(): Promise<boolean> {
      ...
      const newChatBtn = candidates.find(el => {
        ...
        return /新建对话|新对话|New Chat/i.test(text) ||
          /新建对话|新对话|New Chat/i.test(label) ||
          /\/new$/i.test(href)
      })
      ...
      location.href = 'https://yiyan.baidu.com/'
      return true
    },
