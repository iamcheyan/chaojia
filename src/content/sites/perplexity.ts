...
const EDITOR_SELECTORS =
  'textarea[placeholder], div[contenteditable="true"][role="textbox"], textarea[contenteditable="true"], #ppl-search-input'

const SEND_BUTTON_SELECTORS =
  'button[aria-label*="Send"], button[aria-label*="发送"], button[type="submit"], button[aria-label*="送信"], button[aria-label*="送る"], button[data-testid="submit-button"]'

const NEW_CHAT_SELECTORS =
  'a[href="/"], button[aria-label*="New Thread"], button[aria-label*="新对话"], button[aria-label*="新しいチャット"], a[aria-label*="新对话"]'

const RESPONSE_SELECTORS = '.prose, .markdown, [data-testid="message"], [class*="answer"], [class*="response"], [class*="prose"]'

const STOP_SELECTORS =
  'button[aria-label*="Stop"], button[aria-label*="停止"], button[aria-label*="中止"], button[aria-label*="ストップ"], button[data-testid="stop-button"]'

const ACTIVITY_INDICATORS =
  '.animate-pulse, [data-is-streaming="true"], [class*="loading"], [class*="typing"], [class*="generating"]'
...
export function createPerplexityAdapter(): ChatSiteAdapter {
  return {
    id: 'perplexity',
    ...
    async startNewChat(): Promise<boolean> {
      ...
      const newChatBtn = candidates.find(el => {
        ...
        return /new thread|新对话|新しいチャット/i.test(text) ||
          /new thread|新对话|新しいチャット/i.test(label) ||
          /\/$/i.test(href)
      })
