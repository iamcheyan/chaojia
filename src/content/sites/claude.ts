...
const SEND_BUTTON_SELECTORS =
  'button[aria-label*="Send"], button[aria-label*="发送"], button[aria-label*="送信"], button[aria-label*="送る"]'

const NEW_CHAT_SELECTORS =
  'a[href="/new"], button[aria-label*="New chat"], button[aria-label*="新对话"], button[aria-label*="新しいチャット"]'

const RESPONSE_SELECTORS = '[data-is-streaming], .font-claude-message, [data-testid="message"]'

const STOP_SELECTORS =
  'button[aria-label*="Stop"], button[aria-label*="停止"], button[aria-label*="中止"], button[aria-label*="ストップ"]'

const ACTIVITY_INDICATORS =
  '.result-streaming, [data-is-streaming="true"], .animate-pulse'
...
export function createClaudeAdapter(): ChatSiteAdapter {
  return {
    id: 'claude',

    getResponseContainers(): Element[] {
      const candidates = [...document.querySelectorAll(RESPONSE_SELECTORS)]
      // Filter to only assistant messages (Claude marks them with data attributes or structure)
