...
const SEND_BUTTON_SELECTORS =
  'button[aria-label*="发送"], button[aria-label*="Send"], button[type="submit"], button[aria-label*="送信"], button[aria-label*="送る"], [data-testid="chat-input-send-button"]'

const NEW_CHAT_SELECTORS =
  'a[href="/"], button[aria-label*="新对话"], button[aria-label*="New Chat"], button[aria-label*="新しいチャット"], a[aria-label*="新对话"], [class*="new-session"], [class*="new-chat"]'

const RESPONSE_SELECTORS = '.markdown, .prose, [data-testid="message"], [class*="message-content"], [class*="assistant-content"], [class*="chat-content"]'

const STOP_SELECTORS =
  'button[aria-label*="停止"], button[aria-label*="Stop"], button[aria-label*="中止"], button[aria-label*="ストップ"], [data-testid="stop-button"]'
...
export function createTongyiAdapter(): ChatSiteAdapter {
  return {
    id: 'tongyi',
    ...
    async startNewChat(): Promise<boolean> {
      ...
      location.href = 'https://tongyi.aliyun.com/qianwen/'
      return true
    },
