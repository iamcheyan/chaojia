...
// Doubao (豆包) uses a textarea or contenteditable div for input
const EDITOR_SELECTORS =
  'textarea[placeholder], div[contenteditable="true"][role="textbox"], textarea[contenteditable="true"], div[contenteditable="true"]'

// Send button: Doubao uses buttons with Chinese/English aria-labels or a submit-type button
const SEND_BUTTON_SELECTORS =
  'button[aria-label*="发送"], button[aria-label*="Send"], button[type="submit"], button[aria-label*="送信"], button[aria-label*="送る"], button[data-testid="send-button"]'

// New chat button: Doubao sidebar typically has a new conversation button
const NEW_CHAT_SELECTORS =
  'button[aria-label*="新对话"], button[aria-label*="New Chat"], a[aria-label*="新对话"], a[href="/"], button[data-testid="new-chat"]'

// Response container selectors: Doubao uses markdown/prose classes similar to other Chinese AI sites
const RESPONSE_SELECTORS =
  '.markdown, .prose, [data-testid="message"], [class*="message"], [class*="response"], [class*="chat-message"], [class*="assistant"]'

// Stop button selectors
const STOP_SELECTORS =
  'button[aria-label*="停止"], button[aria-label*="Stop"], button[aria-label*="中止"], button[data-testid="stop-button"]'

// Activity/streaming indicators
...
export function createDoubaoAdapter(): ChatSiteAdapter {
  return {
    id: 'doubao',
    ...
    async startNewChat(): Promise<boolean> {
      ...
      const newChatBtn = candidates.find(el => {
        ...
        return /new chat|新对话|新会话|新しいチャット/i.test(text) ||
          /new chat|新对话|新会话|新しいチャット/i.test(label) ||
          /\/new$/i.test(href)
      })
      ...
      // Fallback: navigate to Doubao main page
